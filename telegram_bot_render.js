...
if (!TOKEN || !API_URL || !WEBHOOK_URL) {
    console.error('❌ TOKEN, API_URL ou WEBHOOK_URL não configurados!');
    process.exit(1);
}
...
// Rota de healthcheck (evita hibernação)
...
// Definir webhook na inicialização
async function setupWebhook() {
    try {
        const webhookPath = `${WEBHOOK_URL}/bot${TOKEN}`;
        await bot.setWebHook(webhookPath);
        console.log(`✅ Webhook configurado: ${webhookPath}`);
    } catch (err) {
